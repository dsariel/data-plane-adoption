...
function cleanup_resources() {
  ...
  if ${BASH_ALIASES[openstack]} image show ${IMAGE_NAME} &>/dev/null; then
    ...
    ${BASH_ALIASES[openstack]} image delete ${IMAGE_NAME} || true
    echo "Image deleted."
  else
...
if [[ -z "$BAREMETAL_NODES" ]]; then
  echo "ERROR: No baremetal nodes found after enrollment. Cannot proceed."
  echo "Please check if ENROLL_BMAAS_IRONIC_NODES is properly configured and the node enrollment was successful."
  exit 1
fi
